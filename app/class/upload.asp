...
If admin = 0 Then
   ...
   Response.End()
End If
Dim Fso,msg,pic,picname,mime,ext,path
action   = Request.Querystring("act")
Dim sSavePath : sSavePath = rPath&"assets/file/" & Year(Now) & Right("0"&Month(Now),2) & "/"
If action ="pic" Then 
pic = Request.Querystring("pic")
'DeleteFiles(rPath&pic)
img = Trim(request.Form("img"))
If img="" Then Response.Write "{""error"":0,""url"":""""}":Response.End
Call FolderNameCheck(sSavePath)
picname = "s_"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now()) ''生成当天的子文件夹的名称
Strs = Split(img,",")
mime = Replace(Replace(Strs(0),";base64",""),"data:","")
If mime="image/jpg" Or mime="image/jpeg" Then 
ext = ".jpg"
...
ext = ".jpg"
End if
picture=Strs(1) 'replace(picture,"data:image/png;base64,","")
path=sSavePath&picname&ext
...
Set xml=Nothing
Set stm=Nothing
Response.Write "{""error"":0,""url"":"""&Replace("-"&path,"-"&rPath,"")&"""}"
Else
Call FolderNameCheck(sSavePath)
Dim UpFile
set UpFile = New UpLoadClass
UpFile.AutoSave = 2
UpFile.MaxSize = 1024 * 1024 * 7 '# (1024*1024=1M) SESSION里的单位为KB
UpFile.FileType =  "bmp/gif/jpg/jpeg/png" '允许上传文件类型
UpFile.Charset="UTF-8"
UpFile.SavePath = sSavePath
UpFile.Open() '# 打开对象
If UpFile.Save("picture",0) Then
	'If IsJpeg="1" Then Call CreateView(sSavePath & UpFile.Form("imgFile"))	
	Response.Write "{""state"":""SUCCESS"",""error"":0,""url"":"""&Replace("-"&sSavePath,"-"&rPath,"") & UpFile.Form("picture")&"""}"
Else
	Select Case UpFile.Form("picture_Err")
	Case -1 : msg= "没有文件上传，请重新上传"
	Case 1 : msg=  "文件大小超出限制，请重新上传"
	Case 2 : msg=  "不允许上传的文件类型，请重新上传"
	Case 3 : msg=  "文件大小超出限制并且是不允许上传的文件类型，请重新上传"
	Case Else : msg= "未知错误，请重新上传"
	End Select		
